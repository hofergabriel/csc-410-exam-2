...
We will partition the array into $n/chunk\_size$ subsections or `chunks' where $n$ is the size of the 
...
Therefore, communication is needed to ‘move’ the floating-point numbers to the right tasks/bins. 
...
The messages contain the amount that each non-local task should update their 
...
Since the number of tasks and the number of processors are the same, task $t$
is assigned/mapped to process $p$ such that $t=p$. This mapping was illustrated in Figure 1.
...
The reason the root is pinged before sending $bin\_cnt$ to the root  
is that we need to make sure that every message to update every $bin\_cnt$ has been completed 
(i.e. synchronized). 

\subsection*{Extending our approach to the Stem-and-Leaf Plot}
Let's extend the method for parallelizing the histogram to the stem-and-leaf plot.  
The same parameter $chunk\_size$ will be used to partition the array into smaller subsections. 
...
But, instead of only sending an integer amount to add to other tasks' $bin\_cnt$ variables, 
each message will contain a list of the actual floating-point numbers, 
so that each $target\_task$ can process the numbers they receive from other tasks 
and add them to their local hash-table. 
(We could have done this (sent the floats themselves) with the histogram, 
but it was more memory efficient to send an integer that would update the $bin\_cnt$.)  
...
range of values that belong to its partition.
...
Now I realize that it would have been much easier to extend the 
method used to make the Stem-and-Leaf plot to the Histogram. Once all of the 
floats were sent to their appropriate tasks, we would simply have to count the number 
of floats in a task to calculate $bin\_cnt$, the size of a bin in the histogram. Furthermore, 
Stem-and-Leaf plot can be converted to a Histogram by putting all of the stems/leafs in
