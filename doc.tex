...
count (‘$bin\_cnt$’) variable. See Figure 2. 
...
But, instead of only sending an integer amount to add to the ‘$bin\_cnt$’, each message will contain 
a list of the actual floating-point numbers, so that each task can process the numbers 
and add them to their local hash-table. We could have done the same thing 
(send the floats themselves) for the histogram, but it was more memory efficient to 
just send an integer that would update the $bin\_cnt$.  Using this approach, a single task may 
have between $0$ and $n$ unique stems.  A stem will only exist if there is at least one leaf. 
See Figure 3.

\includegraphics[scale=.6]{/home/gabe/Pictures/csc-410-exam-2/img3.png}
